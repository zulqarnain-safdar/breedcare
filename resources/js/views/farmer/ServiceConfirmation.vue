          ...
          <form @submit.prevent="submitConfirmation" class="space-y-6">
            ...
            <div v-if="confirmationForm.service_completed">
              ...
              <div class="relative">
                ...
                <input
                  v-model.number="confirmationForm.amount_paid"
                  type="number"
                  step="0.01"
                  min="0"
                  class="block w-full pl-7 pr-3 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                  placeholder="0.00"
                />
              </div>
            </div>
  ...
  setup() {
    ...
    const loadJob = async () => {
      try {
        ...
        const response = await window.axios.get(`/technician/jobs/${jobId}`)
    ...
    const submitConfirmation = async () => {
      try {
        ...
        formData.append('amount_paid', confirmationForm.value.amount_paid)
        ...
        alert('Service confirmation submitted successfully!')
